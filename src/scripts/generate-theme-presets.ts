...
const presets = files.map((file) => {
  ...
  const lightPrimaryMatch = content.match(
    /:root\[data-theme-preset=["'][^"']*["']\]\s*\{[\s\S]*?--primary:\s*([^;]+);/,
  );
  const darkPrimaryMatch = content.match(
    /\.dark:root\[data-theme-preset=["'][^"']*["']\]\s*\{[\s\S]*?--primary:\s*([^;]+);/,
  );
...
async function main() {
  const formatted = execFileSync('npx', ['prettier', '--write', '--stdin-filepath', outputPath], {
    input: updated,
    encoding: 'utf8',
  });
